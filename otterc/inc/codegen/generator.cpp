    ...
    namespace codegen {
            ...
            if (detail::sharedIsa<functionAST>(var->Val)) {
                ...
                if (var->Type == TypeID::Int) {
                    constant = llvm::ConstantInt::getSigned(
                        Type::getInt32Ty(this->TheContext), init);
                } else if (var->Type == TypeID::Double) {
                    constant = llvm::ConstantFP::get(
                        Type::getDoubleTy(this->TheContext), init);
                }
                auto gvar = new GlobalVariable(
                    *this->Module,
                    detail::type2type(var->Type, this->TheContext), false,
            ...
            if (auto rawCall = detail::sharedCast<funcCallAST>(expr)) {
                ...
                if (auto args = rawCall->Args.at(0)) {
                    ...
                    if (detail::sharedIsa<identifierAST>(args)) {
                        if (auto rawID =
                                detail::sharedCast<identifierAST>(args)) {
                    ...
                    if (type->getPointerElementType()->getTypeID() == 14) {
                        argsType = PointerType::get(
                            Type::getInt8Ty(this->TheContext), 0);
                        mangling = "puts";
                    } else if (type->getPointerElementType()->getTypeID() ==
                               11) {
                    ...
                    FunctionType* func_type = FunctionType::get(
                        Type::getInt32Ty(this->TheContext), argsType, false);
                    Function* func = dyn_cast<Function>(
                        this->Module->getOrInsertFunction(mangling, func_type));

                    return this->Builder->CreateCall(
                        func, this->Builder->CreateLoad(val));
                }
            }
        }
        ...
        Value* Generator::GeneratorStatement(std::shared_ptr<baseAST> stmt,
            ...
            if (detail::sharedIsa<funcCallAST>(stmt)) {
                ...
                auto value = this->GeneratorGlobalValue(
                    stmt, type, func->getValueSymbolTable());
                return value;
            } else if (detail::sharedIsa<numberAST>(stmt)) {
                if (auto rawNum = detail::sharedCast<numberAST>(stmt)) {
                    if (llvm::Type::getDoubleTy(this->TheContext) ==
                        func->getReturnType()) {
                        return llvm::ConstantFP::get(func->getReturnType(),
                                                     rawNum->Val);
                    } else {
                        return llvm::ConstantInt::get(func->getReturnType(),
                                                      rawNum->Val);
                    }
                }
        ...
        Value* Generator::generateVariable(std::shared_ptr<variableAST> var,
            ...
            return this->Builder->CreateAlloca(
                detail::type2type(var->Type, this->TheContext), constant,
                var->Name);
        }
