...
x3::rule<class variable, std::shared_ptr<variableAST>> const variable(
    "variable");
x3::rule<class statement, std::shared_ptr<statementsAST>> const statement(
    "statement");
...
auto const id_def = x3::lexeme[((x3::alpha | '_') >> *(x3::alnum | '_'))];
...
auto const addExpr_def =
    ...
    *((x3::lit("+")[detail::sharedAdd("+")] >> mulExpr[detail::addAST()]) |
      (x3::lit("-")[detail::sharedAdd("-")] >> mulExpr[detail::addAST()]));
auto const boolExpr_def = addExpr >> *(("=" >> addExpr) | ("<" >> addExpr) |
                                       (">" >> addExpr) |
                                       ("<=" >> addExpr) |
                                       ("=>" >> addExpr) |
                                       (">=" >> addExpr));
