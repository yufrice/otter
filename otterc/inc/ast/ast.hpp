...
struct functionAST : public baseAST {
    ...
    void setVal(const std::string& ast) { Args.emplace_back(std::move(ast)); }

    void addAst(const auto& ast) { Statements.emplace_back(std::move(ast)); }
...
struct moduleAST : public baseAST {
    ...
    void addAst(const auto& ast, auto& type) {
        if (std::is_convertible<decltype(ast),
                                std::shared_ptr<variableAST>>::value == true) {
    ...
    void addAst(const auto& ast) {
        if (std::is_convertible<decltype(ast),
                                std::shared_ptr<funcCallAST>>::value == true) {
